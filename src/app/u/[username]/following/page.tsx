...
export default async function FollowingPage({ params }: Params) {
  ...
  // People this profile follows (newest first)
  ...
  const ids = ((followRows ?? []) as { following_id: string }[]).map(
    (r) => r.following_id
  );
  ...
  return (
    <main className="min-h-screen pt-24 pb-20 px-6">
      <div className="max-w-2xl mx-auto">
        ...
        <h1 className="font-display text-3xl text-white mb-6">Following</h1>
        <PeopleList
          people={people}
          emptyText={`${displayName} isn't following anyone yet.`}
        />
      </div>
    </main>
  );
