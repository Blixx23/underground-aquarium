...
export default function TankVoteControl({
  tankId,
  initialScore,
  initialVote = 0,
  ...
  // Pages can be cached, so the server can't bake in per-user state. On mount
  // we read the viewer's own vote (RLS scopes tank_votes to them) so the
  // control reflects a prior up/down. We never clobber an in-progress vote.
  useEffect(() => {
    let active = true;
    supabase
      .from("tank_votes")
  ...
  async function cast(dir: 1 | -1) {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/tanks/vote", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tank_id: tankId, value: next }),
      });
      if (res.status === 401) {
        setVote(prevVote);
        setScore(prevScore);
